...
int main(int argc, char* argv[])
{
		...
		for (int i = 0; i < procList.size(); i++)
			cout << procList[i].id << ": " << procList[i].timeFinished + 1 << endl;

		// Turnaround Time
		// (service time + wait time)/processes
		float sum = 0;
		cout << "Turnaround Time: " << endl;
		for (int i = 0; i < procList.size(); i++)
		{
			sum += (procList[i].timeFinished + 1) - procList[i].startTime;
			cout << procList[i].id << ": " << (procList[i].timeFinished + 1) - procList[i].startTime << endl;
		}
		cout << "Avg: " << sum/static_cast<float>(procList.size()) << endl;

		// Normalized Turnaround Time
		//  Response Ratio = (W + S)/S
		sum = 0;
		cout << "Normalized Turnaround Time: " << endl;
		for (int i = 0; i < procList.size(); i++)
		{
			sum += (((procList[i].timeFinished + 1) - procList[i].startTime)/static_cast<float>(procList[i].totalTimeNeeded));
			cout << procList[i].id << ": " << (((procList[i].timeFinished + 1) - procList[i].startTime)/static_cast<float>(procList[i].totalTimeNeeded)) << endl;
		}
		cout << "Avg: " << sum/static_cast<float>(procList.size()) << endl;
