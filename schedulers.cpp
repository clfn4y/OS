...
int SPN(const int& curTime, const vector<Process>& procList)
{
	...
	for (int i = 0; i < procList.size(); i++)
		if (procList[i].totalTimeNeeded != procList[i].timeScheduled && procList[i].timeScheduled != 0)
			idx = i;
	// If not find a new proocess
	if (idx == -1)
	{
		for (int i = 0; i < procList.size(); i++)
		{
			if (curTime >= procList[i].startTime && procList[i].isDone == false)
			{
				if (idx == -1)
					idx = i;
				if (procList[i].totalTimeNeeded < procList[idx].totalTimeNeeded)
					idx = i;
			}
		}
	}
...
int SRT(const int& curTime, const vector<Process>& procList)
{
	...
	for (int i = 0; i < procList.size(); i++)
	{
		if (curTime >= procList[i].startTime && procList[i].isDone == false)
		{
			if (idx == -1)
				idx = i;
			if (procList[i].totalTimeNeeded - procList[i].timeScheduled < procList[idx].totalTimeNeeded - procList[idx].timeScheduled)
				idx = i;
		}
	}
	...
	if (idx == -1)
	{
		for (int i = 0; i < procList.size(); i++)
		{
			if (curTime >= procList[i].startTime && procList[i].isDone == false)
			{
				if (idx == -1)
					idx = i;
				if (((curTime - procList[i].startTime) + procList[i].totalTimeNeeded)/static_cast<float>(procList[i].totalTimeNeeded) > ((curTime - procList[idx].startTime) + procList[idx].totalTimeNeeded)/static_cast<float>(procList[idx].totalTimeNeeded))
					idx = i;
			}
		}
	}
